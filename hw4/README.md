...
## Practice #2: Let’s control the music with MediaPipe inputs
Mediapipe: install via pip

```bash
conda activate your_env # optional
pip install mediapipe opencv-python python-osc
```

You can test MediaPipe installation by running the example Python code:

```bash
python mediapipe_to_osc.py
```

This example opens your webcam(required), detects face landmarks with MediaPipe FaceMesh, computes normalized mouth width/height, maps them to `/gesture/mouth/width` (≈10..16) and `/gesture/mouth/height` (≈5..10), and sends them via OSC.

For more details about face landmarks and configuration, see the MediaPipe Face Landmarker guide: [Face Landmarker](https://ai.google.dev/edge/mediapipe/solutions/vision/face_landmarker). You can adjust mapping ranges or extend the script as needed.

If you have any problem regarding MediaPipe installation, see the official installation guide:
[Installing MediaPipe](https://ai.google.dev/edge/mediapipe/framework/getting_started/install)
...
- Use both your hands (MIDI Keyboard or Computer Keyboard) and face (MediaPipe) to make creative sound.
